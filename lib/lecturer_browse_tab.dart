...
class _LecturerBrowseAssetsState extends State<LecturerBrowseAssets> {
  ...
  Widget _buildAssetCard({
    ...
    // --- LOGIC TO CHOOSE IMAGE SOURCE ---
    // If path contains 'uploads', it's a new image from server (Network).
    // Otherwise, assume it's an old local image (Asset).
    final bool isNetworkImage = cleanImagePath.contains('uploads');
            ...
            ClipRRect(
              ...
              child: AspectRatio(
                ...
                child: cleanImagePath.isNotEmpty
                    ? (isNetworkImage
                          ? Image.network(
                              'http://$url/$cleanImagePath', // Fetch from Server
                              fit: BoxFit.contain,
                          ...
                          : Image.asset(
                              cleanImagePath, // Load from Local Assets
                              fit: BoxFit.contain,
                              errorBuilder: (context, error, stackTrace) =>
                                  _buildErrorIcon(),
                            ))
