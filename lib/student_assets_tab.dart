...
class _StudentAssetsTabState extends State<StudentAssetsTab> {
  ...
  Widget _buildAssetCard(
    ...
    // Determine if this is a server upload or a local asset
    final bool isNetworkImage = imagePath.contains('uploads');
    // Create the full URL only if it's a network image
    final String fullImageUrl = isNetworkImage
        ? 'http://$_apiUrl/$imagePath'
        : '';
              ...
              Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) => asset_details.AssetDetailsPage(
                    assetName: name,
                    // Pass full URL if network, else pass local path
                    imagePath: isNetworkImage ? fullImageUrl : imagePath,
                    status: status,
                    assetId: assetId,
                  ),
                ),
